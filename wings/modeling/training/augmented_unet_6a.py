...
config 5's own result (circular mask + rotation_p=0.3, see
augmented_unet_5.py), which fixed the structural augmentation issues but
still plateaus around val_mean_error_px ~2.1-2.2px, held back by a persistent
~3-4% val_wrong_spot_count_pct tail (median error keeps slowly improving even
as mean stalls, suggesting a genuine subset of hard-to-detect landmarks
rather than general stagnation).
...
the *same* key ("criterion.bce.pos_weight") in both config 5's criterion and
